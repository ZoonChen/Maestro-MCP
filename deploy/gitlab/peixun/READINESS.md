...
| P5a-6 | Jira 连通性实测 | ✅（**09-14 内网复测全通 + J3 首演三腿全绿**） | 首测（09-11，ART-incident-001@1）：DNS 正常（172.16.0.114）、HTTPS/ICMP 全超时→回退手工锚定。**复测（09-14，内网）**：`https://jira10.fe-iu.com` HTTP 200（0.24s）；mcp-atlassian 直连公司实例（Server/DC 版，凭据 rd03，实测为 Jira 系统管理员）：读面全通，写面全通（EXAMPLE-1 创建+删除探测，已回收）。**J3 首演（同日，窗口 WIN-20260914-J1-01）**：新建 Jira 项目 **PEIXUN（企业学堂试点，id 10102，REST 简化式创建）**——落点取独立项目而非 AI-14 Epic（看板污染/对账噪音/最小暴露三考量）；A1-1/A1-2 镜像 issue PEIXUN-1/2 + 2 条 manual 锚点；常驻栈 `jira:` 段首启（base_url + `env:MAESTRO_PILOT_JIRA_PAT` + 600s）。三腿实证：**推送**（占位标题→SoR 标题 + `maestro:done` 标签）、**快照**（mirror_ok/snapshot 回填）、**对账**（人为漂移→detect→2 周期 escalated→accept_sor 裁决→re-push→clean 全生命周期）。**S2A 遗留口径**：pre-s2b 恢复带入的 110 条假想锚点（PXPEIXUN-101~261/PXPOC-1~5 指向不存在的 issue）实证为死锚（首周期 404 记账不 abort）后清理——"手工锚定回退"的真实形态=占位 key，真实接入时需清理重锚。TLS：证书=DigiCert 公共 CA，但容器 `SSL_CERT_FILE` 单文件语义要求 pilot-stack `certs/ca.pem` 扩为 pilot CA+公共根 bundle（OIDC 依赖不受影响）。治理缺口登记：rd03 为系统管理员凭据（蓝图"只读 PAT"预设不成立），V4 前应换专用受限账号 |
