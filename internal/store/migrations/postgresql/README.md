...
19. `0022` D1 三门控制面自证（任务书 D1 / F14 结构解其一，权威：docs/quality/gates-and-evidence.md §3 control_plane 语义 + docs/specs/schemas/evidence.schema.json 条件规则）：唯一既有表变更是 `evidence.authority` CHECK 扩 `'control_plane'`——评估引擎对三门（policy_integrity/baseline_freshness/boundary）的机械判定以 authority=control_plane 的 append-only 行入 evidence 表（判定权归引擎、与 CI producer 平权聚合 EVIDENCE-RULE-003、无 pipeline/job 身份）。自证行由评估器铸造：deterministic identity（同判定同 ID 幂等重放）+ attempt 递增（判定变更=新行，最新 attempt 胜出，旧行留史）。down 为 best-effort：先删 control_plane 行再收 CHECK（判定下次评估按旧引擎重铸）。偏差登记：(a) 0021（W5 契约清理）未登记本台账，编号顺延；(b) gate_snapshots/waivers 无结构变更——自证证据走既有表既有列。
