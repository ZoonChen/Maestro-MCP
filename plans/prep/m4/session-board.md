...
### 影子期全流程任务书（2026-09-12 下发，跑通影子项目全部工作）

| 顺序 | 会话 | 任务书 | 内容 | 前提 |
|---|---|---|---|---|
| 1 | S2A | brief-S2A-bom-import.md | BOM→WorkGraph 全量导入（一期 44 建图+锚定，二三期 46 占位）+ 幂等导入脚本 | 无（常驻栈在） |
| 2 | S2B×N | brief-S2B-dev-slices.md | **可重复模板**：开发切片在治理下流动（领取→详设→沙箱实现→MR→证据→人工合并→done→Jira 同步）；首切片 A1-1+A1-2 | **S2A 执行完毕** |
| 3 | S2C | brief-S2C-observation-exit.md | 周报自动化 + 零干扰分类 + 出口评估包（灰度决策输入） | S2A 后建采集；评估需 ≥2 周数据 |
...
### W6 收口：灰度三硬前提全关（2026-09-13，会话 I）

- **W6（#123）合入**（+2622/34 文件，五项 CI 全绿）：A1 done 链写者（机器自有边+审计/outbox 同事务）/A2 分支契约项目段权威解析/A3 四 API 端点（S2B 驱动器正式退役）/A4 多会籍显式 scope/A5 DomainEventSink（孤儿租约回收）/A6 SLO 窗口聚合+stale 回退。**W1 复现翻绿对比全过**（validating 2→0、done 0→2、evidence 0→24、投影 0→2、outbox 172+9 孤儿→0）。spec：writes 32→36、MCP 目录 3.3→3.4、slo-schema +stale、RBAC 68 不变。
- **灰度就绪核对单三硬前提打钩**；剩余非代码前提：≥2 周周报（最早 09-26 出口评估）、常驻栈重建+库恢复（见事故）、试点侧 F5/F6/F7。
- **⚠ live maestro 库第三次被清空**（W6 会话发现，时间窗在 S2C 周报后，根因同族=共享栈无会话隔离）：未单方处置；恢复点=`pre-s2b-20260913-111737.dump`（S2A 全量 110 项）；S2B/S2C 会话期数据需按交接物重建；建议随常驻栈重建窗口一并处理并**升级 ART-incident-003 行动项（库隔离/自动备份）为灰度前置**——灰度 Agent 写真实缺陷流转，库不可再裸奔。

### P3 下发：共享栈保护（三次数据丢失根治 + 库恢复，灰度硬前置）

| 会话 | 任务书 | 内容 | 时机 |
|---|---|---|---|
| P3 | brief-P3-shared-stack-guard.md | 三道防线（物理隔离/逻辑防线/恢复底线）+ 主库从备份恢复 + 排查结论存档 | **灰度硬前置**，立即 |
