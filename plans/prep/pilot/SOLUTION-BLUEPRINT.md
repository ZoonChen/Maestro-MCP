...
### 1.3 账号与角色映射（owner 确认 2026-09-14：按表执行，技术负责人兼 technical_lead 职能成立）
...
### 2.1 等保 2.0 三级 ↔ Maestro 控制项映射（内部使用期暂不执行，保留映射做三期参考）

| 等保要求族 | Maestro 已有控制项 | 差距 **[待评审]** |
|---|---|---|
| 安全审计（留存≥6 个月） | 只追加审计链 + 导出/验证（#88）；audit_retention_days=365 冻结 | ~~审计归档异地存储~~ **owner 决策（2026-09-14）：内部使用期间暂不考虑等保**；三期启动时重评 |
...
### 3.2 运维责任矩阵（owner 决策 2026-09-14：内部使用期间暂不指定专门运维；平台操作由集成会话执行，恢复备份由 owner 自行触发）
...
| 决策块 | 状态 | 评审人 | 批注时间 |
|---|---|---|---|
| §1.1 SoR 矩阵 / §1.2 同步语义 / §2.2 数据分级 / §2.3 组件评审 / §3.1 部署演进 / §4.3 推广门槛（指标名） | 集成会话写实 | 集成会话 | — |
| §2.1 等保差距 | **owner 批注：内部使用期暂不考虑** | owner（陈自恒） | 2026-09-14 |
| §3.2 运维矩阵 | **owner 批注：内部使用期暂不指定专门运维** | owner（陈自恒） | 2026-09-14 |
| §4.2 授权书签发人 | **owner 批注：签发人=陈自恒** | owner（陈自恒） | 2026-09-14 |
| §4.3 推广门槛阈值 | **owner 批注：按推荐执行** | owner（陈自恒） | 2026-09-14 |
| §1.3 角色映射 | **owner 确认：按表执行，技术负责人兼 technical_lead 成立** | owner（陈自恒） | 2026-09-14 |
