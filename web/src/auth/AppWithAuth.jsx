...
// blank screen. While the backend exposes no /auth routes the probe
// answers 404 and the console keeps its read-only anonymous shape.
export function AppWithAuth() {
  const { session, retry } = useAuthSession(AUTH_CONFIG);

  useEffect(() => {
    const onExpired = () => { retry(); };
  ...
  return <AuthGateView config={AUTH_CONFIG} session={session} retry={retry} />;
}
